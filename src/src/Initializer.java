...
public class Initializer
{
	...
	numbPALADIN = settings.numbPerGroup;
	numbFIGHTER = settings.numbPerGroup;
	numbROGUE = settings.numbPerGroup;
	numbRANGER = settings.numbPerGroup;
	numbWIZARD = settings.numbPerGroup;
	numbCLERIC = settings.numbPerGroup;
	numbSKELETON = settings.numbPerGroup;
	numbORC = settings.numbPerGroup;
	...
	else if (settings.n == 3)
	{
		...
		for (int z = 0; z < settings.M; z++)
		{
			...
			position[0] = (int) settings.pixel / 2 + x * settings.pixel;
			position[1] = (int) settings.pixel / 2 + y * settings.pixel;
			position[2] = (int) settings.pixel / 2 + y * settings.pixel;
			chanceOfCharacterPlacement_Evenly(molecules, position);
		    }
		}
	    }
    ...
    private void chanceOfCharacterPlacement_Evenly(ArrayList<Character> molecules, double[] position)
    {
	...
	{
	    molecules.add(new Character(CharacterClass.PALADIN, position));
	    numbPALADIN--;
	}

	else if (random.nextDouble() <= numbFIGHTER / M2 && numbFIGHTER > 0)
	{
	    molecules.add(new Character(CharacterClass.FIGHTER, position));
	    numbFIGHTER--;
	}

	else if (random.nextDouble() <= numbROGUE / M2 && numbROGUE > 0)
	{
	    molecules.add(new Character(CharacterClass.ROGUE, position));
	    numbROGUE--;
	}

	else if (random.nextDouble() <= numbRANGER / M2 && numbRANGER > 0)
	{
	    molecules.add(new Character(CharacterClass.RANGER, position));
	    numbRANGER--;
	}

	else if (random.nextDouble() <= numbWIZARD / M2 && numbWIZARD > 0)
	{
	    molecules.add(new Character(CharacterClass.WIZARD, position));
	    numbWIZARD--;
	}

	else if (random.nextDouble() <= numbCLERIC / M2 && numbCLERIC > 0)
	{
	    molecules.add(new Character(CharacterClass.CLERIC, position));
	    numbCLERIC--;
	}

	else if (random.nextDouble() <= numbSKELETON / M2 && numbSKELETON > 0)
	{
	    molecules.add(new Character(CharacterClass.SKELETON, position));
	    numbSKELETON--;
	}

	else if (random.nextDouble() <= numbORC / M2 && numbORC > 0)
	{
	    molecules.add(new Character(CharacterClass.ORC, position));
	    numbORC--;
	}
