...
public class LevelUp
{
	...
	if (character.getType() == CharacterClass.PALADIN)
	    Paladin(character, level);
	if (character.getType() == CharacterClass.FIGHTER)
	    Fighter(character, level);
	if (character.getType() == CharacterClass.ROGUE)
	    Rogue(character, level);
	if (character.getType() == CharacterClass.RANGER)
	    Ranger(character, level);
	if (character.getType() == CharacterClass.WIZARD)
	    Wizard(character, level);
	if (character.getType() == CharacterClass.CLERIC)
	    Cleric(character, level);

	if (character.getType() == CharacterClass.SKELETON)
	    Skeleton(character, level);
	if (character.getType() == CharacterClass.ORC)
	    Orc(character, level);
	...
	if (level == 0)
	{
	    ...
	    character.AC = 10 + 9 + 4 + (character.DEX - 10) / 2; // H.Armor,
	    // H.Shield
	...
	if (level == 0)
	{
	    ...
	    character.AC = 10 + 9 + 4 + (character.DEX - 10) / 2;
	    character.Flat = 10 + 9 + 4;
	    character.Touch = 10 + (character.DEX - 10) / 2;
